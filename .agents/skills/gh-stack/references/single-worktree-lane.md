...
If you maintain this workflow as an agent skill, keep the skill directory inside the
repo/worktree skill directory:
...
Create one lane worktree from the integration branch. Run this from an existing clean
integration worktree:

```bash
cd <container-root>/<integration-worktree>
git worktree add ../feature-<feature-slug>-lane -b feature-<feature-slug>-svc-1 <integration-branch>
...
```bash
cd <container-root>/feature-next-report-lane
